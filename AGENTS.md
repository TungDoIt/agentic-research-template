...
Save a concise setup checkpoint with status `not started`, a timestamp with timezone, and setup as the last completed action. Leave research findings and the report explicitly uninvestigated. Briefly summarize the saved setup; no routine approval step is required. Begin research when asked to start the loop, or in the same turn if the user already requested both setup and research.
